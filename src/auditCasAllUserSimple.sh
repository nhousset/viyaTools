	...
	{
	   echo ""
	   echo -e "${YELLOW}Usage: $0 -s <sleep time> -o <output dir> -c <cas server>${NC}"
	   echo "       $0 -s 10 -l \"/tmp\" -c default"
	   echo ""
	   echo -e "\t-s SLEEP_TIME"
	   echo -e "\t-o LOG_PATH"
	   echo -e "\t-c CAS_SERVER"
	...
	do
		...
		then
			...
			echo "nomServeur;FrenchDate;processUser;CASSessionID;NB_FILE_CACHE;SIZE_IN_CACHE;DF_CACHE_USED;LOAD;CPU;processPID;processCPU;processMEM;processRSS;ProcessVSZ;nbPidCasSession"  > $LOG_FILE
		fi
		...
		do
			...
			echo $nomServeur";"$FrenchDate";"$sasUser";"$CASSessionID";"$processPID";"$NB_FILE_CACHE";"$SIZE_IN_CACHE";"$LOAD";"$CPU";"$processPID";"$processCPU";"$processMEM";"$processRSS";"$processVSZ  | tee -a $LOG_FILE
