...
do
	...
	do
		...
		CASSessionID=$(echo $process | cut -d ";" -f 8)
		 
		somme_VSZ=somme_VSZ+$processVSZ
		somme_RSS=somme_RSS+$processRSS
		
		NB_FILE_CACHE=$(ls -lrt /proc/${processPID}/fd | grep $CAS_DISK_CACHE_PATH | wc -l)
		...
		echo $nomServeur";"$FrenchDate";"$sasUser";"$CASSessionID";"$NB_FILE_CACHE";"$SIZE_IN_CACHE";"$DF_CACHE_USED";"$LOAD";"$CPU";"$processPID";"$processCPU";"$processMEM";"$processRSS";"$processVSZ";"$nbPidCasSession";0;0;0;0"  | tee -a $LOG_FILE
