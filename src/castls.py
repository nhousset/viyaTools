...
def main():
    ...
    if tag == 5:
        wrappedSocket = ssl.wrap_socket(cas.cas, server_side=False, cert_reqs=ssl.CERT_NONE, ciphers=None)
        fp = tempfile.NamedTemporaryFile()
        fp.write(str.encode(ssl.DER_cert_to_PEM_cert(wrappedSocket.getpeercert(True))))
        fp.seek(0)
